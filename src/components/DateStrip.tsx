...
export const DateStrip: React.FC<DateStripProps> = ({ selectedReturn = false }) => {
  ...
  return (
    <div className="mx-auto flex max-w-5xl items-center gap-3 overflow-x-auto px-4 pb-8">
      <button className="h-14 shrink-0 rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white">&lt; - 2 days</button>
      {prices.map((price, index) => (
        <button
          key={price}
          className={`h-20 min-w-28 rounded-xl border px-3 text-center text-sm font-semibold ${
            index === 3 ? 'border-[#073b70] bg-[#073b70] text-white ring-4 ring-blue-200' : 'border-slate-300 bg-white text-slate-600'
          }`}
        >
          <span className="block text-xs font-medium opacity-70">{price.split(' ').slice(0, 2).join(' ')}</span>
      ...
      <button className="h-14 shrink-0 rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white">+ 7 days &gt;</button>
    </div>
  );
};
