    ...
    <main className="min-h-screen bg-slate-100 text-slate-800">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <Link to="/services" className="hidden h-11 items-center rounded border border-slate-300 px-4 text-sm font-semibold text-slate-600 hover:bg-slate-50 sm:inline-flex">
              Services
            </Link>
      ...
      <section className="bg-[#073b70] px-6 py-14 text-white">
        <div className="mx-auto max-w-7xl">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-200">Flight Status</p>
          <div className="mt-5 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
            <div>
              <h1 className="text-4xl font-semibold tracking-normal sm:text-6xl">Track Your Journey</h1>
            ...
            <div className="rounded-lg border border-white/15 bg-white/10 p-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-cyan-100">Latest check</p>
              <p className="mt-2 text-2xl font-semibold">{lastUpdated ? lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "Waiting for search"}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-10 lg:grid-cols-[360px_1fr]">
        <aside>
          <form onSubmit={checkStatus} className="rounded-lg border border-slate-300 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3 border-b border-slate-200 pb-5">
              ...
              <div>
                <p className="text-sm font-semibold uppercase tracking-widest text-[#073b70]">Search Flight</p>
            ...
            <label htmlFor="flight-number" className="mt-6 block text-xs font-semibold uppercase tracking-widest text-slate-500">
              Flight Number
              <input
                id="flight-number"
                value={flightNumber}
                ...
                placeholder="HE742"
                autoComplete="off"
                className="mt-2 h-12 w-full rounded border border-slate-300 px-4 text-base font-medium uppercase text-slate-800 outline-none focus:border-[#073b70] focus:ring-1 focus:ring-[#073b70]"
              />
            </label>

            <label htmlFor="departure-date" className="mt-5 block text-xs font-semibold uppercase tracking-widest text-slate-500">
              Departure Date
              <input
                id="departure-date"
                value={departureDate}
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-6 flex h-13 min-h-13 w-full items-center justify-center gap-2 rounded bg-[#073b70] px-5 py-4 font-semibold text-white transition hover:bg-[#052b52] disabled:cursor-not-allowed disabled:opacity-70"
            >
          ...
          <div className="mt-6 rounded-lg border border-slate-300 bg-white p-6 shadow-sm">
            <div className="flex gap-3">
              ...
              <div>
                <h2 className="font-semibold uppercase tracking-wide text-[#073b70]">Travel Advisory</h2>
        ...
        <section aria-live="polite">
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">Live Information</p>
              <h2 className="mt-1 text-3xl font-semibold text-[#073b70]">Latest Update</h2>
            </div>
          ...
          {!error && loading && (
            <div className="flex min-h-96 flex-col items-center justify-center rounded-lg border border-slate-300 bg-white p-8 text-center text-slate-500 shadow-sm">
              ...
              <p className="text-xl font-semibold text-[#073b70]">Retrieving flight status</p>
          ...
          {!error && !loading && !hasSearched && (
            <div className="flex min-h-96 flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500 shadow-sm">
              ...
              <p className="text-2xl font-semibold text-[#073b70]">Search a flight to begin</p>
          ...
          {!error && !loading && hasSearched && results.length === 0 && (
            <div className="rounded-lg border border-amber-300 bg-amber-50 p-8 text-center text-amber-800 shadow-sm">
              <p className="text-xl font-semibold">No matching flight found</p>
            ...
            return (
              <article key={`${flightLabel}-${index}`} className="mb-6 overflow-hidden rounded-xl border border-slate-300 bg-white shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-7 py-6">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">{flight.airline?.name || "Airline unavailable"}</p>
                    <h3 className="mt-1 text-3xl font-semibold text-[#073b70]">Flight {flightLabel}</h3>
                  </div>
                  <span className={`rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-widest ${statusClasses(flight.status)}`}>
                ...
                <div className="grid gap-8 p-7 lg:grid-cols-[1fr_220px_1fr] lg:items-center">
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-4 inline-flex items-center gap-2 rounded bg-slate-100 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
...
function FlightEndpoint({
  ...
  return (
    <div className={align === "right" ? "lg:text-right" : ""}>
      ...
      <p className="mt-2 text-4xl font-semibold text-[#073b70]">{airportCode(endpoint)}</p>
      <p className="mt-2 text-sm font-semibold text-slate-500">{airportName(endpoint, fallbackName)}</p>
      <p className="mt-4 inline-flex rounded bg-blue-50 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-[#073b70]">
  ...
  return (
    <div className="flex gap-3 rounded border border-slate-200 bg-white p-4">
      ...
      <div>
        ...
        <p className="mt-1 font-semibold text-[#073b70]">{value}</p>
      </div>
    </div>
  );
}
